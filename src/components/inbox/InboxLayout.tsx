...
interface InboxLayoutProps {
  ...
  /** Optional back arrow target — set to a route for the page version, omit to hide. */
  backTo?: string;
  /** Whether to embed in a flex parent or set its own min-height. */
  embedded?: boolean;
}
...
export function InboxLayout({
  ...
  }, [activeId, list, filtered]);

  useEffect(() => {
    if (selected && activeId !== selected.id) {
      setLocalId(selected.id);
  ...
  return (
    ...
    >
        ...
        >
          {backTo && (
            <Link
              ...
              style={{ width: 30, height: 30, borderRadius: 9, color: "#3A3A34", marginLeft: -6 }}
            >
              <ArrowLeft size={16} strokeWidth={1.8} />
            </Link>
          )}
          <h1
            style={{ fontSize: 18, fontWeight: 700, letterSpacing: "-.02em", color: "#1B1B17" }}
          >
            Inbox
          </h1>
          {urgentCount > 0 && (
      ...
      <section className="flex flex-1 flex-col" style={{ background: "#FFFFFF", minWidth: 0 }}>
        {selected ? (
          <DetailPane
            item={selected}
        ...
        {!hideHeader && (
          ...
          >
            ...
            <div className="flex shrink-0 items-center gap-2">
              ...
              {onExpand && <ExpandIconButton onClick={onExpand} ariaLabel="Expand item" />}
              <button
                type="button"
                onClick={onClose}
                aria-label="Close"
                className="flex items-center justify-center hover:bg-[#F4F3EE]"
                style={{ width: 32, height: 32, borderRadius: 9, color: "#3A3A34" }}
              >
